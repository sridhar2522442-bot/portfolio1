...
import { Map, MousePointer2, Smartphone } from 'lucide-react';
...
export default function Home() {
  const { hasEntered, activeHotspot, viewMode, setViewMode, setActiveHotspot, deviceTier } = useStore();
  ...
  return (
    <main className="w-full h-full relative bg-bg-void text-text-primary overflow-hidden">
      ...
      {/* Fullscreen Monitor UI (About Me) */}
      <AnimatePresence>
        {viewMode === 'fullscreen-monitor' && (
          <motion.div
            ...
            onClick={() => { setViewMode('room'); setActiveHotspot(null); }}
          >
            {/* Scanline */}
            ...
            <button
              ...
              onClick={(e) => { e.stopPropagation(); setViewMode('room'); setActiveHotspot(null); }}
            >
              <span>⤢</span> Exit
            </button>

            <div className="relative z-10 w-full max-w-2xl px-6 max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={(e) => e.stopPropagation()}>
              <AboutPanel />
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {hasEntered && viewMode !== 'fullscreen-monitor' && (
          <>
            ...
            {/* Map / device tier badge */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="absolute top-6 right-6 flex items-center gap-2 z-30"
            >
              {/* Device tier indicator (dev only) */}
              {process.env.NODE_ENV === 'development' && (
                <div className="px-2 py-1 rounded-full bg-black/60 border border-white/10 text-[9px] font-mono text-white/40">
                  {deviceTier.toUpperCase()}
                </div>
              )}
              <button
                className="p-3 rounded-full bg-black/40 border border-white/10 hover:bg-white/10 transition-colors backdrop-blur-md"
                aria-label="Room Map"
              >
            ...
            <AnimatePresence>
              {showHint && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: 10 }}
                  className={`absolute left-1/2 -translate-x-1/2 flex items-center gap-2 bg-black/60 border border-white/10 px-4 py-2 rounded-full backdrop-blur-md pointer-events-none z-30 ${isMobileView ? 'bottom-24' : 'bottom-8'}`}
                >
                  {isMobileView
                  ...
                  <span className="font-mono text-xs text-text-muted">
                    {isMobileView ? 'Tap objects or use nav below' : 'Click objects to explore'}
                  </span>
                </motion.div>
              )}
            </AnimatePresence>
