...
export default function Hotspot({ id, label, position, cameraTarget, cameraPosition, children, isFullscreenAction }: HotspotProps) {
  ...
  const { activeHotspot, setActiveHotspot, hasEntered, setViewMode } = useStore();
  ...
  // Lift the object slightly on hover
  useEffect(() => {
  ...
  const handleClick = (e: any) => {
    ...
    if (!isActive) {
      ...
      if (isFullscreenAction) {
        ...
        setTimeout(() => {
          setViewMode('fullscreen-monitor');
        }, 1100);
      }

      // Dispatch custom event for the camera controller
  ...
  return (
    ...
    >
      ...
      {/* Floating Label & Glow Marker */}
      {hasEntered && !isAnyActive && (
        <Html 
          position={[0, 1.5, 0]} 
          center 
          className={`pointer-events-none transition-opacity duration-300 ${hovered ? 'opacity-100' : 'opacity-0 md:opacity-0 opacity-80'}`}
        >
